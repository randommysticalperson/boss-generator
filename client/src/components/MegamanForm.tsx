...
interface Props {
  ...
  onAddToQueue?: () => void;
  isLoading: boolean;
  onRandomizeAndGenerate?: (data: MegamanFormData) => void;
}

export default function MegamanForm({ onGenerate, onFormChange, onAddToQueue, isLoading, onRandomizeAndGenerate }: Props) {
      ...
      {onRandomizeAndGenerate && (
        <motion.div custom={7} variants={fieldVariants} initial="hidden" animate="visible">
          <button
            ...
            style={{
              background: isLoading ? undefined : "linear-gradient(135deg, oklch(0.55 0.22 250 / 0.25), oklch(0.50 0.24 295 / 0.25))",
              border: "1px solid oklch(0.65 0.22 250 / 0.5)",
              color: "oklch(0.85 0.18 250)",
              boxShadow: isLoading ? undefined : "0 2px 16px oklch(0.65 0.22 250 / 0.2)",
            }}
          >
